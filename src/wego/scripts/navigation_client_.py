...
| MISSION 값   의미                          
| --------- | --------------------------- 
| `0`       | SLAM 위치 초기화 및 목표 이동         
| `1`       | 장애물 회피 주행                   
| `2`       | 고정 조향각 주행 (turn control 기반) 
| `3`       | 로터리 진입 판단 및 정지              
| `4`       | 로터리 탈출 및 라인 기반 주행           
...
0.5101052630588911
    ...
    def mission1(self, msg: LidarObstacleInfoArray): # 장애물 구간
        ...
        if not len(obstacle_infos): 
            if self.obstacle_type == 's' and self.obstacle_point > 0:
                print('복귀~')
                self.return_ok = True
        ...
        dists = np.array([np.hypot(info.obst_x, info.obst_y) for info in obstacle_infos if info.obst_y > 0.15])
        if not len(dists) :
            return
        ...
        if not self.obstacle_type and dist < 1.1:
            ...
            self.obstacle_type = 's' if abs(angle) < 0.13 else 'd'
            ...
            if info.obst_y < 0.2:
                print('if info.obst_y < 0.2, stop_flag = False')
                self.stop_flag = False
                return

            if angle < -(np.pi / 12):
                print('if angle >= np.pi / 9: stop_flag = False', angle)
                self.stop_flag = False
                return

            if dist > 0.73: # 너무 멀면 살짝 앞으로 이동
                print('if dist > 0.7: # 너무 멀면 살짝 앞으로 이동 self.stop_flag = False')
                self.target_vel = self.OBST_VEL
                self.stop_flag = False
                return
            
            print('정적', info, self.obstacle_point)
            if self.obstacle_point == -1 or self.return_ok:
                self.stop_flag = False
                if self.return_ok and (np.pi / 18) < angle:
                    print('718')
                    return
    ...
    def stop_lane_callback(self, msg: Int32):
        ...
        if self.stop_lane_cnt == 1 and self.MISSION != 4:
            ...
            if abs(self.min_dis.dis) < 1.: # 도로에 차량이 있는 경우라면 정지
                print('차량이 앞에 있어요', self.min_dis.dis)
                self.stop_flag = True
                self.stop(f'차량이 앞에 있어요 {self.min_dis.dis}')

            else:
                print('로터리를 탈출해요', self.min_dis.dis)
        ...
        if self.stop_lane_cnt > 1:
            ...
            yaw = self.get_yaw_from_orientation(self.now_orientation)
            # 정방향을 보고, 신호등이 33번 신호가 아니라면
            if 17 / 18 * np.pi <= abs(yaw) and self.traffic_sign != 33:
                # print('신호등을 봤어요')
                self.stop(f'정지선을 한 번 더 봤어요 {self.stop_lane_cnt}, {self.traffic_sign}', )
                self.stop_flag = True
    ...
    def run(self):
        ...
        if self.target_vel == self.LANE_DRIVE_VEL or self.target_vel == self.LANE_DRIVE_LAST_VEL:
            ...
            # M - m error
            steering_angle = self.mapping(np.clip(diff * 1.7, -np.pi / 6, np.pi / 6), -np.pi / 6 , np.pi / 6, 1, 0)
            ...
            if self.MISSION == 4:
                ...
                steering_angle = self.mapping(steering_angle)

        if not self.stop_flag:
            # print('최종 속도', self.target_vel, '최종 조향각', steering_angle)
    ...
    def republish_initialpose(self): # 초기 위치 재설정 함수 
